\documentclass[12pt]{article}

\usepackage{nopageno}
\usepackage[letterpaper, portrait, margin=1in]{geometry}
\usepackage{newtxtext,newtxmath}
\usepackage{titlesec}
\usepackage{hanging}
\usepackage[table,xcdraw]{xcolor}
\usepackage{qtree}


\setlength{\parindent}{0.5in}

\titleformat{\section}
{\normalfont\fontsize{12}{15}\bfseries}{\makebox[0.5in][l]{\thesection}}{0pt}{}
\titleformat{\subsection}
{\normalfont\fontsize{12}{15}\bfseries}{\makebox[0.5in][l]{\thesubsection}}{0pt}{}
\titleformat{\subsubsection}
{\normalfont\fontsize{12}{15}\bfseries}{\makebox[0.5in][l]{\thesubsubsection}}{0pt}{}

\titlespacing*{\section}{0pt}{12pt}{0pt}
\titlespacing*{\subsection}{0pt}{12pt}{0pt}
\titlespacing*{\subsubsection}{0pt}{12pt}{0pt}


\newcounter{data}
\newenvironment{data}
{\refstepcounter{data}\bigskip\par\noindent%
	\makebox[0.5in][l]{(\thedata)}}
	{\bigskip\par\noindent\ignorespacesafterend}

\newcounter{letter}
\newenvironment{letter}
{\refstepcounter{letter}\makebox[0.5in][l]{\theletter.}}{}


\newcommand{\titletext}[1]
{\begin{center}
		\textbf{#1}
\end{center}}

\newcommand{\headertext}[4]
{\begin{flushleft}
		#1
		\\ #2
		\\ #3
		\\ #4
\end{flushleft}}

\newenvironment{references}
{\begin{hangparas}{0.5in}{1}}
	{\\ \end{hangparas}}


\begin{document}

\headertext{Author Name}{Course Title}{Professor Name}{Date}

\titletext{My Title}

\section{test}
this is a test of the data format

with a second paragraph to compare indents
\begin{data}
screm I hate spacing

\begin{letter}
	testing again
\end{letter}
\end{data}
and now there's a paragraph after it

\section{References}
%\begin{references}
%\end{references}
	
\end{document}